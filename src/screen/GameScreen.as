package screen 
{
	...
	public class GameScreen extends Sprite implements IAnimatable
	{
		...
		public function show():void
		{
			...
			{
				var tex:Texture = Asset.getBaseTexture(BackgroundAsset.BG_TILE);
				var img:*;
				var quadBatch:QuadBatch = new QuadBatch();
				var fillX:int = Util.appWidth / tex.width +1;
				var fillY:int = Util.appHeight / tex.height + 1;
				img = new Image(tex);
				for (var j:int = 0; j < fillX; j++) 
				{
					for (var k:int = 0; k < fillY; k++) 
					{
						img.x = img.width * j;
						img.y = img.height * k;
				...
				bgContainer.addChild(quadBatch);
				img.width = Util.appWidth;
				img.height = Util.appHeight;
				
				tex = Asset.getBaseTexture(BackgroundAsset.BG_TILE_HEADER);
				quadBatch = new QuadBatch();
				img = new Image(tex);
				img.height = 160;
				fillX = Util.appWidth / tex.width +1;
				for (var l:int = 0; l < fillX; l++) 
				{
					img.x = img.width * l;						
					quadBatch.addImage(img);
				}
		...
		private function initTopBar():void 
		{
			if (!topBar)
			{
				...
				var startButton:BaseButton = ButtonAsset.getBaseBt(ButtonAsset.BT_CYAN);
				startButton.x = 300;
